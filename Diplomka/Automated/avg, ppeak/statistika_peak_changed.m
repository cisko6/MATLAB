    ...
    for l=2:4
        ...
        for o=1:3
            ...
            if typ_statistiky == "dkl"
                ...
                close(figure1)
            elseif typ_statistiky == "chi"
                saveas(figure1,fullfile(average_folder_path,sprintf('CHI-STATISTICS, compute_window=%d, chi_alfa=%.2f, pocet_tried_hist=%d, shift=%d.png',compute_window,chi_alfa,pocet_tried_hist, shift)));
                saveas(figure1,fullfile(average_folder_path,sprintf('CHI-STATISTICS, compute_window=%d, chi_alfa=%.2f, pocet_tried_hist=%d, shift=%d.fig',compute_window,chi_alfa,pocet_tried_hist, shift)));
                close(figure1)
            end

            clearvars -except M slot_window predict_window compute_window sigma_nasobok cely_tok file_path folder_path where_to_store attacks_folder_mat folder_name posun_dat shift pocet_tried_hist chi_alfa simulacia use_fourier keep_frequencies typ_statistiky;    
            close all;
        end
    end
end
